  ...
  <el-main>
    ...
    <el-table :height="tableHeight" :data="tableData" border stripe>
      ...
      <el-table-column label="操作" fixed="right" width="550">
        <template slot-scope="scope">
          <span class="span-button-text"  v-permission="['sys:reader:edit']" @click="editBtn(scope.row)">编辑</span>

          <span @click="applyBtn(scope.row)" class="span-button-text" v-permission="['sys:reader:apply']"
             v-if="scope.row.checkStatus == '0'">同意</span>

            <span class="span-button-text" v-permission="['sys:reader:resetpassword']" type="warning" @click="resetPasswordBtn(scope.row)">
              重置密码
            </span>
            <span class="span-button-text" v-permission="['sys:reader:delete']" type="danger" @click="deleteBtn(scope.row)">
              删除
            </span>
    ...
    <sys-dialog :title="dialog.title" :width="dialog.width" :height="dialog.height" :visible="dialog.visible"
      ...
      <div slot="content">
        <el-form ref="addRef" :model="addModel" :rules="rules" label-width="80px" style="margin-right: 30px">
          ...
          <!-- 密码框仅在用户类型为0时显示 -->
          <el-form-item v-if="addModel.type == '0'" prop="password" label="密码">
